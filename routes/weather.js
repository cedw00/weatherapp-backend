...
router.post('/', (req, res) => {
	...
	City.findOne({ cityName: { $regex: new RegExp(req.body.cityName, 'i') } }).then(dbData => {
		if (dbData === null) {
			...
			fetch(`https://api.openweathermap.org/data/2.5/weather?q=${req.body.cityName}&appid=${process.env.API_KEY}&units=metric`)
...
router.delete("/:cityName", (req, res) => {
    City.findOne({ cityName: req.params.cityName }).then(data => {
        if (data === null) {
            ...
            City.deleteOne({ cityName: req.params.cityName }).then(city => {
